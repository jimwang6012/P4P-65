...
def main():
    ...
    for file in arr:
        try:
            ...
            os.makedirs(f'../csnippex-apizator/{dictionary[post_id]}/{post_id}/com/stackoverflow/api', exist_ok=True)
            p = subprocess.Popen(['java', '-jar', 'rename-class.jar', '../csnippex-apizator/output-apizator-java/' + file,
                                  '../csnippex-apizator/output-csnippex-java/' + file, f'../csnippex-apizator/{dictionary[post_id]}/{post_id}/com/stackoverflow/api/'], start_new_session=True)
            p.wait(timeout=5)
